...
ssize_t  file_read(struct file *fd, char __user *buf, size_t size, loff_t *offset)
{
 ...
 */
//int jit_currenttime(char *buf, char **start, off_t offset, int len, int *eof, void *data)
ssize_t  file_read_cur(struct file *fd, char __user *buf, size_t size, loff_t *offset)
{
	struct timeval tv1;
	struct timespec tv2;
	unsigned long j1;
...
struct file_operations proc_fops_cur=
{
    .read=file_read_cur,
    .owner=THIS_MODULE,
};
...
ssize_t jit_timer(struct file *fd, char __user *buf, size_t size, loff_t *offset)
{
...
struct file_operations proc_fops_jitimer=
{
    .read=jit_timer,
    .owner=THIS_MODULE,
};
...
ssize_t jit_tasklet(struct file *fd, char __user *buf, size_t size, loff_t *offset)
{
	struct jit_data *data;
	char *buf2 = buf;
	unsigned long j = jiffies;
...
struct file_operations proc_fops_jit_tasklet=
{
    .read=jit_tasklet,
    .owner=THIS_MODULE,
};
...
{
	proc_create_data("curtime", 0, NULL, &proc_fops_cur, NULL);
	proc_create_data("jitbusy", 0, NULL, &proc_fops, (void *)JIT_BUSY);
/*
...
*/
	...
	proc_create_data("jitimer", 0, NULL, &proc_fops_jitimer, NULL);
	proc_create_data("jitasklet", 0, NULL, &proc_fops_jit_tasklet, NULL);
	proc_create_data("jitasklethi", 0, NULL, &proc_fops_jit_tasklet, (void *)1);
...
{
	remove_proc_entry("curtime", NULL);
