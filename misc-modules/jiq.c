...
ssize_t jiq_read_wq(struct file *fd, char __user *buf, size_t size, loff_t *offset)
{
	DEFINE_WAIT(wait);
...
ssize_t jiq_read_wq_delayed(struct file *fd, char __user *buf, size_t size, loff_t *offset)
{
	DEFINE_WAIT(wait);
...
struct file_operations proc_fops_jiqwq={
	.read=jiq_read_wq,
	.owner=THIS_MODULE,
};
struct file_operations proc_fops_jiqwqdelay={
...
static int jiq_init(void)
{
	...
	proc_create_data("jiqwq", 0, NULL, &proc_fops_jiqwq, NULL);
	proc_create_data("jiqwqdelay", 0, NULL, &proc_fops_jiqwqdelay, NULL);
