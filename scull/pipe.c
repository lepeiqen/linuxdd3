...
int scull_p_init(dev_t firstdev)
{
	...
	result = register_chrdev_region(firstdev, scull_p_nr_devs, "scullp"); //lpq allocate requested deviceNumber
	if (result < 0) {
	...
	for (i = 0; i < scull_p_nr_devs; i++) {
		...
		scull_p_setup_cdev(scull_p_devices + i, i);//lpq connect requested deviceNum to deviceOps
	}
#ifdef SCULL_DEBUG
